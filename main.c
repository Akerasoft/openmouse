...
#ifdef WITH_MOUSE
		gamepad = mouseGetGamepad();
		gamepad->update();
		gamepad->getReport(&paddata);

		if (paddata.pad_type == PAD_TYPE_MOUSE) {
			initial_controller = PAD_TYPE_MOUSE;
			wm_setAltId(adapter_snes_id);
		} else if (paddata.pad_type == PAD_TYPE_SNES) {
...
#ifdef WITH_MOUSE
	mouse_gamepad = mouseGetGamepad();
	default_gamepad = mouse_gamepad;
#endif
