...
void dataToClassic(const gamepad_data *src, classic_pad_data *dst, char first_read)
{
	...
	switch (src->pad_type)
	{
		...
		case PAD_TYPE_MOUSE:
			...
			memcpy(dst->controller_raw_data, src->mouse.raw_data, MOUSE_RAW_SIZE);
			mouse_x = src->mouse.raw_data[2];
			mouse_y = src->mouse.raw_data[3];

			if (mouse_x & 0x80 == 0x80)
			{
				dst->lx = -(mouse_x & 0x7F);
			}
			else
